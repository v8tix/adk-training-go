...
Every custom function tool so far has been a Go function you write by hand, wrapped with `functiontool.New`. Great when you know the parameters at compile time. But what if you want to expose *any* REST operation as a tool — described by data rather than code (a name, a URL, a list of parameters) — and get a working tool back without writing a wrapper function for each one? That's exactly what this module builds. 🏗️
...
`openapitool.NewToolset("frankfurter", frankfurterSpec)` turns that data straight into a working tool — no per-endpoint Go function to write. 🎉
...
Adding a second operation to the same toolset is just adding a second `OperationSpec` to the `NewToolset` call — no second wrapper function needed.

### Handling a Real API's Errors

A live REST API can fail a few different ways, and they call for different responses. Confirmed against the real Frankfurter API: a bad currency code returns a normal HTTP response — `404` with `{"message":"not found"}` — exactly the kind of thing the model should read and explain, not something that should crash the agent. A genuine network failure is different: there's nothing for the model to reason about, so that becomes a real Go `error` instead. `openapitool`'s `Run` makes that split explicit — the same distinction module-9's divide-by-zero handling made.

`Run` also checks for a missing required parameter *before* ever calling the real API — caught in review, since a local model occasionally skips one, and silently calling the API without it could give a wrong answer instead of an obvious error (Frankfurter itself defaults a missing `from` to EUR rather than rejecting the request 😬). That check produces the same kind of structured result as an API error, so the model has one consistent shape to read regardless of which of the two problems hit.

### Key Takeaways ✅
- A tool can be built from data (a spec) instead of a hand-written function — great whenever the parameter set isn't known until runtime.
