...
Your job: verify this repo's Go environment is ready for ADK 2.0 development. If you get stuck, work backward from the Troubleshooting section below. 🕵️

## Lab Tasks

### Step 0: Confirm Go 1.27+ (Crucial) 🔍
...
If it reports anything below `go1.27`, grab a newer Go from `https://go.dev/dl/` before continuing.
...
By default, `cmd/verify-setup` calls a local Ollama server — no `.env` file needed. Want to check Google Cloud/AI Studio credentials instead?
...
Open `cmd/verify-setup/main.go` and `checks.go`, plus `internal/infrastructure/llm/config.go` and `factory.go` (moved there in module-3 so a second program could reuse them — see that module's docs for why). See if you can find:
