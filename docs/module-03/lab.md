...
Your job: build and run a simple "Echo" agent using the Go ADK SDK.
...
4. No `.env` configuration is required for the default (local Ollama) path. Want to test against Gemini instead? Copy `.env.example` to `.env` and set `MODEL_TYPE=gemini` plus `GOOGLE_AI_STUDIO_API_KEY`.
5. Run the agent:
