...
No `cmd/launcher` import at all — this is a plain Go program driving the agent directly:
...
### 4. Verify no regression in `cmd/support-analyzer` ✔️
...
- In a real Go web server (built on `net/http`, say), where would you construct the `*runner.Runner` — inside each request handler, or once at startup as a shared variable? Why does that match Python's own guidance for its `Runner` singleton?
