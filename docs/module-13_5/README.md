# Module 13.5: Extending ADK — Custom Persistence with Redis (Go)
...
Every prior module used `session.InMemoryService()` without a second thought. It's just one implementation of a plain Go interface:
...
Nothing about `runner.Runner` cares which implementation it's handed — you inject your own via `runner.Config`:
...
From this point on, every `r.Run(...)` call persists through your storage, without touching a single line of agent instruction or tool code.

### `session.Service`'s Contract Has No Default Implementation

`session.Service` is a plain Go interface — implementing it means writing every method's full behavior yourself, including how a `StateDelta` gets applied to state. Confirmed by reading the SDK's own `InMemoryService` directly: that application isn't a flat copy. A `StateDelta` key is scoped three ways:
...
A custom `session.Service` has to implement this scoping itself — there's no shared default to call into. `internal/infrastructure/redissession`'s `AppendEvent` does exactly that:
...
Python's lab verifies its Firestore provider by running it once and eyeballing the result. Go has something stronger: `session/sessiontestsuite`, a real, backend-agnostic conformance suite the SDK ships and uses to test its *own* built-in SQL-backed service:
...
This is what actually caught the three-tier state scoping above, live, while building this module: an earlier version treated all state as session-private, and `RunServiceTests` failed two subtests (`app_state_is_shared`, `user_state_is_user_specific`) with a precise, actionable diff — not a guess about what might be wrong.

### A Real Backend, Verified with Testcontainers

Python's lab requires a real Google Cloud project and `gcloud auth application-default login`. This course has never required a full cloud account before, so this module substitutes **Redis** instead — named explicitly in Python's own Theory section as a valid alternative for exactly this use case ("near-instant response times a standard persistent database might not provide"). `internal/infrastructure/redissession`'s tests use [Testcontainers](https://testcontainers.com/) to start a real, ephemeral Redis container — no manually-managed test database, no mocks standing in for the real thing, and nothing left running after the test suite exits.

### Key Takeaways
- `session.Service` is a plain interface, injected into `runner.Config.SessionService` — any implementation works, with zero changes to agent or tool code.
...
- `session/sessiontestsuite.RunServiceTests` verifies a custom implementation against the same suite the SDK uses on its own built-in service — a stronger correctness story than manual spot-checking.
- Testcontainers provisions a real, ephemeral backend for integration tests — the first module in this course to need it.
