...
* **`go run ./cmd/echo-agent web --port 9091 webui api`** — starts a local Dev UI at `http://localhost:9091/ui/` (note the flag position: `web`'s own flags go directly after `web`, then the sub-launcher keywords; the lab uses `9091` instead of the launcher's default `8080`, which is commonly already occupied by other local dev tools — see the lab for a confirmed real collision). **Both `webui` and `api` are required together** — confirmed live in module-5: the Dev UI's frontend calls the REST API (`api`) for everything beyond serving its own static page, so `webui` alone starts a UI that 404s the moment you try to actually use it. This is the Go equivalent of `uv run adk web`.
