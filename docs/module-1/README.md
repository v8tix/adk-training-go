...
| **App & Runner** — the infrastructure layer | No hand-built `Runner` here (unlike Python's `InMemoryRunner`). You wrap your root agent/workflow with `agent.NewSingleLoader(...)`, hand it to a `launcher.Config`, and let `google.golang.org/adk/v2/cmd/launcher` (`full.NewLauncher()`) execute it — the launcher picks dev-UI, CLI, or API-server mode from the command line. An `App` type exists for advanced cases (plugins, caching, lifecycle) but isn't needed for a first agent. |
...
- Go's execution model runs through `cmd/launcher`, not a hand-built `Runner` — that's the main thing to unlearn if you're coming from the Python version of this course.
