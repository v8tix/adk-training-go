...
You've now built six real, working multi-agent systems in this repo, each solving orchestration a genuinely different way. Choosing the right geometry for a new graph — before writing any code — is the most important decision this section of the course teaches.
...
Each link goes to that module's own README — this milestone's job is the recap and the decision framework, not a second copy of six modules' worth of Theory.

### How to Choose, in Go's Own Terms

Three questions, translated from the abstract into what they actually mean once you're holding Go code:
...
Where does **Collaborative Teams** (19) fit? It's the odd one out on purpose: not about graph *shape* at all, but about how much control you cede to the sub-agent itself. `Mode: ModeSingleTurn`/`ModeTask` gives you LLM-driven delegation with a *guaranteed* return, without writing any orchestration code — the right choice when a specialist's own judgment about when it's "done" is good enough, and you'd rather not hand-write the loop or the routing edge yourself.

### Details Worth Keeping in Mind

Two of this repo's own six modules confirmed something genuinely worth remembering beyond just "how to build the pattern":
...
Both are the kind of detail that determines whether your own test (or your own trace-inspection code) actually proves what you think it proves. Module-21's own review caught exactly this category of mistake in a *test*, not just documentation: an assertion that checked only an event's author, not really whether the call had succeeded.

### Key Takeaways
- There is no one-size-fits-all architecture — most of this repo's own later, larger builds mix these patterns (a static fan-out feeding a dynamic loop, say).
...
- Every pattern this milestone recaps has a real, live-tested Go package in this repo — when in doubt about a mechanism's exact behavior, that package's own tests and README are the ground truth, not this recap table.
