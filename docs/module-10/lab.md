...
Two handlers: `storeName` writes to `ctx.State()` under the key `"user_name"`; `recallName` reads it back, returning `"Stranger"` when nothing has been stored yet (`errors.Is(err, session.ErrStateKeyNotExist)`). Notice `recallName`'s parameter type, `RecallNameArgs struct{}` — empty, not omitted, since `functiontool.New` requires a struct or map even for a tool that takes no data.

### 2. Read `internal/agents/memory/agent.go`

Same shape as `internal/agents/calculator/agent.go` — two `functiontool.New` calls, attached via `Tools`. No forced backend; this agent works against the local default just like the calculator did.
...
Two separate turns in the same session — the second answer proves `recall_name` actually read back what `store_name` wrote in the first turn, not that the model just remembered "Mario" from the raw chat text (the instruction's `# Constraints` section requires the tool be used either way — see `internal/agents/memory/agent_test.go` for the real, structural test of this).
...
`TestMemory_RemembersNameAcrossTurns_Ollama` and `_Gemini` — both call the agent **twice**, via two separate `Run()` calls against the same session ID, and assert the second call's answer mentions "Mario". This is deliberately not one `Run()` call with two messages — the point is proving state survives *across* `Run()` boundaries, the actual behavior this module is about.

## Self-Reflection Questions
- Why is it more reliable to store data in `agent.Context.State()` rather than just relying on the LLM's own chat history?
...
- `agent.StrictContextMock` panics on any method you don't override. Why is that a better default for a test double than silently returning a zero value?
