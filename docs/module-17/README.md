...
### A Lighter-Weight Alternative to a Fully Dynamic Graph

Module-16's edges are fixed at construction time — the graph's shape never changes at runtime. A fully dynamic graph (this course's own later module) lets code decide, step by step, which node runs next. Structured routing sits between the two: the graph's *shape* is still fixed — every possible branch is declared up front as an edge — but *which* branch actually fires depends on a value one node computes while the graph runs. One classification step, several possible destinations, decided by data rather than hand-written control flow.
