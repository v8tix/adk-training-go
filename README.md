...
**Current coverage:** modules 1 through 29 — everything from your first agent through distributed, multi-agent systems over real A2A network calls, plus state and memory, artifacts, evaluation, observability, RAI safety plugins, callbacks/guardrails, building/consuming MCP stateful tools, and custom UI integration. See the [full module list](#-course-outline) below.
...
| **A reachable Redis** (default `localhost:6379`, override with `REDIS_ADDR`) | Module 13.5's own live demo (`cmd/persistent-agent`) — separate from Docker above, this one needs an actual persistent instance, not Testcontainers' ephemeral one | `docker run -p 6379:6379 redis` (quickest), or a native install from [redis.io](https://redis.io/docs/latest/operate/oss_and_stack/install/) |
